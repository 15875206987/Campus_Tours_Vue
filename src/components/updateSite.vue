<template>
  <div>
      ...
      <Row :gutter="32">
        <Col span="12">
          ...
          <FormItem label="Longitude" label-position="top">
            <Input v-model="formData.site_lng" placeholder="please enter site's longitude" disabled/>
          </FormItem>
        </Col>
        <Col span="12">
          <!--              纬度-->
          <FormItem label="Latitude" label-position="top">
            <Input v-model="formData.site_lat" placeholder="please enter site's latitude" disabled/>
          </FormItem>
        </Col>
      </Row>
    ...
    <div class="demo-drawer-footer">
      <Button type="primary" @click="updateSite">update site</Button>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods:{
      updateSite(){
        this.$store.dispatch('updateSite',this.formData)
      },
      before(file) {
        this.file = file;
